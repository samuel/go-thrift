...
var (
	...
	invalidPackageNameCharsRE = regexp.MustCompile(`[^a-zA-Z0-9_]`)
)
...
func tidyPackageName(name string) string {
	return invalidPackageNameCharsRE.ReplaceAllString(strings.ToLower(name), "_")
}
...
func (g *GoGenerator) Generate(name string, out io.Writer) (err error) {
	...
	packageName = tidyPackageName(packageName)
